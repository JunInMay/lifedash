...
플러그인 기반 개인 대시보드 데스크톱 앱입니다. 사용자는 시계, 타이머, 메모, TODO, 링크, 번역기, 사전, 시장 지표, 주식, 뉴스, AI 채팅, Teams, YouTube, 브라우저, 동영상 재생기, 리코셰 로봇 같은 위젯형 플러그인을 자유롭게 추가하고 배치할 수 있습니다.
...
| 기본 플러그인 | 시계, 타이머 |
| 지원 플러그인 | AI 채팅, 브라우저, 시계, 사전, 바로가기, 시장 지표, 뉴스, 메모, 리코셰 로봇, 종목 검색, Teams, 타이머, TODO, 번역기, 동영상 재생기, YouTube |
...
| 데스크톱 셸 | [[Electron]] | 42 | 데스크톱 앱 실행, `<webview>`, IPC 브리지 |
...
| 레거시 | Tauri | 2 | Electron 마이그레이션 후 git 히스토리에만 보존 |
...
브라우저에서 프론트엔드만 확인할 때는 다음 명령을 사용합니다. 에이전트 검증은 사용자 실사용 포트 1430 대신 1435를 씁니다.

```powershell
npm run dev -- --port 1435
```
