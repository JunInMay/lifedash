...
// Tauri child webview로 진짜 youtube.com을 카드 본문 위에 띄운다.
...
function YoutubePlugin({ instanceId }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const label = `yt-${instanceId.slice(0, 8)}-${Date.now().toString(36)}`;
  ...
  return (
    ...
    >
      {status === "loading" && <div style={{ color: "#8b93a3" }}>유튜브 로딩 중...</div>}
      {status === "ready" && <div style={{ color: "#5b6270" }}>유튜브가 이 위에 표시됩니다</div>}
      {status === "error" && (
