...
- 입력란 높이가 늘어날 때 메시지 목록은 `flex: 1 1 auto`로 줄어들고 자체 스크롤을 유지한다. 입력 영역이 마지막 메시지를 덮으면 안 되므로 `.plugin-body`/`.chat-root`는 `min-height: 0`과 padding 포함 높이 계산(`box-sizing: border-box`)을 유지해야 한다. 또한 입력 리사이즈 직후 메시지 목록의 `scrollTop`을 다시 바닥으로 맞춘다.
...
- **2026-06-15 (Codex)**: 자동 높이 입력란이 커질 때 메시지 목록을 덮어 마지막 응답이 보이지 않던 레이아웃 버그 수정. `.plugin-body`와 aichat flex 컨테이너의 `min-height`/`overflow`/`box-sizing`을 보정하고, 입력 리사이즈 직후 `.chat-msgs` 스크롤을 바닥으로 다시 맞추도록 변경. 검증: `npm run build` 통과.
